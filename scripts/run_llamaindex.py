"""
LlamaIndex Pipeline — Framework Comparison Runner
-------------------------------------------------
Uses LlamaIndex's SentenceSplitter (token-based sentence splitting)
over flattened chat text to evaluate how default sentence node parsing
performs on conversational WhatsApp exports.
...
def build_llamaindex_index(chat_name: str, model: SentenceTransformer):
    ...
    print(f"  [LlamaIndex] {chat_name}: {len(messages)} messages -> {len(chunks)} nodes")
...
def main():
    ...
    for q in eval_set:
        ...
        if chat not in indexes:
            print(f"[*] Building LlamaIndex index for chat: {chat}")
            embeddings, chunks = build_llamaindex_index(chat, model)
    ...
    out_path = "eval_results/results_llamaindex.json"
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump(results, f, indent=2, ensure_ascii=False)

    print(f"\n[+] Successfully saved {len(results)} LlamaIndex evaluated queries to: {out_path}")
